  ...
  <div class="container-fluid">
    ...
    <div class="row">
      <div class="col-12 col-sm-6 col-md-12 mb-3">
        ...
        @endif
        <a href="{{ route('experts.create')}}" class="btn btn-success btn-block">Create a new Expert</a>
       <div class="table-responsive">
            <table class="table">
                <tr>
                    ...
                    <th>Expertise</th>
                    <th>Expert Image</th>
                    <th>Expert Facebook URL</th>
                    <th>Expert Instagram URL</th>
                    <th>Expert Twitter URL</th>
                    <th>Expert LinkedIn URL</th>
                    <th>Status</th>
                    <th>Actions</th>
                </tr>
                ...
                <td><img src="{{asset('back/images/uploads/experts/'.$expert->image)}}" style="height:50px; width:50px;" alt=""></td>
                <td>{!!$expert->facebook_url!!}</td>
                <td>{!!$expert->instagram_url!!}</td>
                <td>{!!$expert->twitter_url!!}</td>

                <td>{!!$expert->linkedin_url!!}</td>
                <td>
                                        <label class="switch">
                                         <input type="checkbox" id="togBtn" <?php echo $expert->status== 'active' ? ' checked' : ''; ?> class="website_product_sell" value="<?php echo $expert->id ?>">
                ...
                <td><a  href="javascript:void(0)" onclick="delete_expert({{$expert->id}})" class="btn btn-danger btn-sm"><i class="fas fa-trash" aria-hidden="true"></i></a>
                <a href="{{route('experts.edit',$expert->id)}}" class="btn btn-warning btn-sm"><i class="fas fa-pencil-alt"></i></a>
                <a href="{{route('experts.show',$expert->id)}}" class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i></a></td>
...
<script>
  var element = document.getElementById("experts_sidebar");
...
<script>
        ...
        $('.website_product_sell').click(function(){
            ...
            var expert_status = $(this).val();
          
         

            $.ajax({
                url:"{{route('change_expert_status')}}",
                method:'get',
                data:{
                    "_token": "{{csrf_token()}}",
